...
const MemberEditForm = ({ initialData, onUpdated, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put("/api/members", updatedMember);
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div>
        <label>비밀번호:</label>
        <input type="password" name="passwd" value={form.passwd} onChange={handleChange} />
      </div>
      <div>
        <label>비밀번호 확인:</label>
        <input type="password" name="passwd2" value={form.passwd2} onChange={handleChange} />
      </div>
      <div>
        <label>이름:</label>
      ...
      {guardians.length > 0 && guardians.map((g, i) => (
