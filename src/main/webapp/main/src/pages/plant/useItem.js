...
const useItem = (memberno, plantno, fetchGrowth) => {
  ...
  const useItemEffect = async (itemType) => {
    const config = {
    ...
    const usedKey = `used_${itemType}_${today}`;
    const usedCount = Number(localStorage.getItem(usedKey)) || 0;

    if (usedCount >= config[itemType].limit) {
      alert(`${itemType}는 오늘 ${config[itemType].limit}회까지만 사용 가능합니다.`);
      return;
    }

    const success = await adjustPoint(config[itemType].point);
    if (!success) return;

    try {
      await axios.post('/api/plant/increase', {
        plantno,
        amount: config[itemType].growth,
        reason: itemType,
      });
